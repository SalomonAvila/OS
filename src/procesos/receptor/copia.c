...
struct Report reportBuffer[10000];
int bufferReportIndex = 0;
...
int ejecutarPrestamo(struct InformacionHiloAuxiliar1 *estructura)
{
    int encontradoL = 0;
...
{
  int *actualizadoLibro = malloc(sizeof(int));
...
sem_t semaphoreReportBuffer;

void addToReportBuffer(struct Report report)
{
  sem_wait(&semaphoreReportBuffer);
  if (bufferReportIndex < 10000)
  {
    reportBuffer[bufferReportIndex] = report;
    bufferReportIndex++;
  }
  else
  {
  ...
  sem_post(&semaphoreReportBuffer);
}

void generateReport()
{
  sem_wait(&semaphoreReportBuffer);
  for (int i = 0; i < bufferReportIndex; i++)
  {
    struct Report *r = &reportBuffer[i];
    printf("%c, %s, %d, %d, %s", r->operation, r->nombre, r->isbn, r->ejemplar, r->fecha);
  }
  sem_post(&semaphoreReportBuffer);
}

void *operacionesSegundoHilo(void *ptr)
{
  char opt = *(char *)ptr;
  if (opt == 's')
  {
  ...
  else
  {
    ...
    printf("  r: Generar reporte de operaciones\n");
  }
}
...
int main(int argc, char *argv[])
{
  ...
  struct Report *temporalReport = malloc(sizeof(struct Report));
  ...
  sem_init(&semaphoreReportBuffer, 0, 1);
  ...
  while (true)
  {
    ...
    while (read(pipe_fd, &msg, sizeof(msg)) > 0)
    {
      ...
      case 'D':
        sem_wait(&semaforo);
        temporalReport.operation = 'D';
        strcpy(temporalReport.nombre, msg.nombre);
        temporalReport.isbn = msg.isbn;
        strcpy(temporalReport.fecha, actualDate);
        ...
        if (bufferIndex < 10)
        {
          ...
          *(infoHilo->msg) = msg;
          ...
          int *ejemplar_resultado;
          pthread_join(hiloAux, (void **)&ejemplar_resultado);
      ...
      case 'R':
        sem_wait(&semaforo);
        temporalReport.operation = 'R';
        strcpy(temporalReport.nombre, msg.nombre);
        temporalReport.isbn = msg.isbn;
        strcpy(temporalReport.fecha, editDate);
      ...
      case 'P':
        ...
        *(infoHilo->msg) = msg;
        strcpy(infoHilo->nombreArchivo, fileDatos);
        int ejemplar_resultado = ejecutarPrestamo(infoHilo);
          temporalReport.ejemplar = ejemplar_resultado;
        addToReportBuffer(temporalReport);
